...
## 📲 Dynamic Home Screen Checklist Widget

HabitFlow includes a highly interactive **Home Screen Widget** that allows users to monitor their current streak and check off today's habits directly from their launcher without launching the app:
...
│  Daily Checklist                             │
│ ──────────────────────────────────────────── │
│  [✓] Go for a 5km Run             FITNESS    │
│  [ ] Read 10 Pages of Book        LEARNING   │
│  [✓] Practice Meditation           HEALTH     │
└──────────────────────────────────────────────┘
```

* **Interactive Remote List Collection**: Built using a custom `RemoteViewsService` and `RemoteViewsFactory`. It loads today's active habits asynchronously from the Room SQLite database, avoiding any main-thread blocking.
* **Instant Toggle Actions**: Tapping anywhere on a habit card row inside the widget triggers a custom broadcast pending intent. The `HabitsWidgetProvider` intercepts this, toggles database completions, and pushes instantaneous visual updates.
* **Smart Synchronicity**: Any completions toggled inside the main Compose application instantly run `triggerWidgetUpdate()` inside the ViewModel, guaranteeing absolute consistency between the app UI and the launcher widget.
* **Adaptive styling & Brand Badges**: Styles layout wrappers with a dark cosmic background card design (`@drawable/bg_widget`), glowing checklists, and categorizes badges using color codes parsed directly from active database models.
